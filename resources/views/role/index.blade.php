<x-app-layout title="Role">
        ...
        <div class="flex gap-4 py-0.5 border-b border-gray-400 min-w-[800px]">
            ...
            <div class="w-60">
                <div class="z-40 flex gap-2">
                    <x-action-a href="" x-data="" @click.prevent="$dispatch('open-modal', 'edit-user-{{ $key }}')"
                    ...
                    @endif
                    <x-modal :show="false" :title="'Edit User ' . $item->name" name="edit-user-{{ $key }}">
                        <form action="{{ route('users.update', ['user' => $item->id]) }}" method="post">
                            ...
                            <div class="flex items-end gap-4">
                                <div class="w-full">
                                    ...
                                    <x-text-input id="nama" readonly class="block w-full mt-1" type="text" name="nama"
                                        :value="old('nama', $item->name)" autofocus autocomplete="nama" />
                                ...
                                @if ($item->isNot(Auth::user()))
                                <x-action-a class="z-50 cursor-pointer bg-rose-500" x-data=""
                                    @click.prevent="$dispatch('close');$dispatch('open-modal', 'hapus-user-{{ $key }}')">
                                    <x-action-label>Hapus Akun</x-action-label>
                                    <svg width="24px" height="24px" viewBox="0 0 24 24" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                    ...
                    <x-modal name="hapus-user-{{ $key }}" :title="'Hapus Akun ' . $item->email">
                        <form method="post" action="{{ route('users.destroy', ['user' => $item->id]) }}" class="p-6">
                            @csrf
                            @method('delete')
    ...
    <x-modal :show="false" :title="'Tambah User Baru'" name="tambah-user">
        <form action="{{ route('users.store') }}" method="post">
            ...
            <div class="grid grid-cols-3 gap-x-4 gap-y-2">
                <div class="w-full">
                    <x-input-label for="nama" :value="__('nama')" />
                    <x-text-input id="nama" class="block w-full mt-1" type="text" name="name" :value="old('nama')"
                ...
                <div class="w-full">
                    ...
                    <x-select-input id="role" class="text-base" name="role_id">
                        ...
                        <x-select-option :selected="$item->id == old('role_id')" value="{{ $item->id }}">{{ $item->nama
                            }}</x-select-option>
                        @endforeach
                    </x-select-input>
    ...
    <div class="">
        ...
        <div class="flex items-center gap-4 py-1 border-b border-gray-400">
            ...
            <div class="w-32">
                <div class="z-40 flex gap-2">
                    <x-action-a href="" x-data="" @click.prevent="$dispatch('open-modal', 'edit-role-{{ $key }}')"
                        class="bg-yellow-300 ">
            ...
            <form action="{{ route('role.update', ['role' => $item->slug]) }}" method="post">
                ...
                <div class="w-full">
                    ...
                    <x-text-input id="nama" class="block w-full mt-1" type="text" name="nama"
                        :value="old('nama', $item->nama)" required autofocus autocomplete="nama" />
                ...
                <div class="mt-4 space-y-3">
                    ...
                    // dd($hak);
                    @endphp
                    @foreach ($hakAkses as $itm)
                    <label class="flex items-center mb-2 space-x-2">
                        <input type="checkbox" @if (in_array($itm->id, $hak)) checked @endif
                        name="option[]" value="{{ $itm->id }}"
                        class="w-4 h-4 rounded-md text-mine-200 focus:caret-mine-100">
                        <span class="text-sm font-semibold">{{ $itm->nama }} </span>
                    </label>
                    @endforeach
        ...
        <x-modal name="hapus-role-{{ $key }}" :title="'Hapus Role ' . $item->nama">
            <form method="post" action="{{ route('role.destroy', ['role' => $item->slug]) }}" class="p-6">
                @csrf
                @method('delete')
