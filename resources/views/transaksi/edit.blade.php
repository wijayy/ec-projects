...
<x-app-layout title="Edit Transaksi {{ $transaksi->nomor_transaksi }}">
    ...
    <form action="{{ route('transaksi.update', ['transaksi' => $transaksi->slug]) }}" enctype="multipart/form-data"
        method="post" class="flex flex-wrap gap-4 md:flex-nowrap" x-data='productManager(@json($transaksi->transaksiDetail))'
        x-init="init()">
        @csrf
        @method('put')
                ...
                <div class="w-full">
                    ...
                    <x-select-input name="provinsi_id" id="provinsi_id">
                        @foreach ($provinsi as $item)
                            <x-select-option value="{{ $item->id }}" :selected="old('provinsi_id', $transaksi->provinsi_id) == $item->id">{{ $item->nama }}
                            </x-select-option>
                        @endforeach
                    </x-select-input>
            ...
            <div class="mt-4">
                <template x-for="(item, index) in items" :key="index">
                    <div class="flex items-end justify-between gap-2">
                        ...
                        <x-action-a class="p-1 m-1 cursor-pointer size-10 bg-rose-500"
        ...
        addItem() { this.items.push({ id:
            ...
            removeItem(index) { this.items.splice(index, 1); } }'
                ...
                <!-- Input Fields -->
                <div
                    class="grid items-end grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4 2xl:grid-cols-9 xl:grid-cols-6">
                    <template x-for="(item, index) in items" :key="index">
                        <div class="relative" x-data="{
                            ...
                            <div class="relative flex w-full mt-1 text-center rounded-md shadow-md aspect-square">
                                ...
                                <div
                                    ...
                                    <x-action-a class="bg-rose-500" x-data=""
        ...
        <div class="sticky w-full space-y-4 md:w-1/3" x-data="{ status: 'selesai', diskon: 0, lunas: 1 }">
            ...
            <div class="w-full">
                ...
                <x-text-input id="diskon" x-model="diskon" class="block w-full mt-1" type="number"
                    name="diskon" :value="old('diskon', $transaksi->diskon)" required autofocus autocomplete="diskon" />
            ...
            <div class="w-full">
                ...
                <x-select-input name="platform" id="platform">
                    @foreach ($platform as $key => $item)
                        <x-select-option value="{{ $key }}" :selected="old('platform', $transaksi->platform) == $key">{{ $key }}
                        </x-select-option>
                    @endforeach
                </x-select-input>
