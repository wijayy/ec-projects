...
<x-app-layout title="Semua Transaksi">
        ...
        <div class="flex items-center gap-4">
            ...
            <form method="GET" class="flex items-center gap-4 w-fit">
                ...
                <x-select-input class="m-0" required id="tahun" name="tahun" class="w-20 ">
                    ...
                    <x-select-option :selected="$i == (request('tahun') ?? date('Y'))" value="{{ $i }}">{{ $i }}
                    </x-select-option>
                    @endfor
                </x-select-input>

                <x-action-a class="p-3 bg-mine-200" x-data="" @click.prevent="$el.closest('form').submit()">
        ...
        <div class="flex items-center w-full h-20 p-4 rounded-lg min-h-40 shadow-mine">
            <div class="flex items-center justify-center w-1/4 h-full">
                <svg fill="#000000" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg"
                    xmlns:xlink="http://www.w3.org/1999/xlink" width="64px" height="64px" viewBox="0 0 76.991 76.992"
                    xml:space="preserve">
                    <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                    <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                    <g id="SVGRepo_iconCarrier">
    ...
    <form class="flex flex-wrap items-end w-full gap-4 mt-4 md:flex-nowrap h-fit">
        <div class="grid w-full grid-cols-2 gap-4 lg:w-3/4 lg:grid-cols-4">
            ...
            <div class="w-full">
                ...
                <x-select-input id="provinsi" class="text-base" name="provinsi">
                    ...
                    <x-select-option :selected="$item->slug == request('provinsi')" value="{{ $item->slug }}">{{
                        $item->nama }}</x-select-option>
                    @endforeach
                </x-select-input>
            </div>
            <div class="w-full">
                ...
                <x-select-input id="status" class="text-base" name="status">
                    ...
                    <x-select-option :selected="$item == request('status')" value="{{ $item }}">{{ $item }}
                    </x-select-option>
                    @endforeach
                </x-select-input>
            </div>
            <div class="w-full">
                ...
                <x-select-input id="platform" class="text-base" name="platform">
                    ...
                    <x-select-option :selected="$key == request('platform')" value="{{ $key }}">{{ $key }}
                    </x-select-option>
                    @endforeach
                </x-select-input>
            </div>
        ...
        <div class="flex gap-4 w-fit flex-nowrap">
            ...
            <x-primary-a href="{{ route('transaksi.index') }}" class="bg-mine-300 hover:bg-mine-300">Reset</x-primary-a>
        </div>
    </form>
                ...
                <tr class="gap-4 space-y-4 text-xs text-center rounded-lg md:text-base">
                    ...
                    <td>
                        <div class="flex gap-2">
                            <x-action-a href="{{ route('transaksi.show', ['transaksi' => $item->slug]) }}" x-data=""
                                class="bg-mine-200 ">
                            ...
                            <x-action-a x-data="" href="{{ route('transaksi.edit', ['transaksi' => $item->slug]) }}"
                                class="bg-yellow-400">
    ...
    @foreach ($transaksi as $item)
    @if ($item->status != 'selesai')
    <x-modal name="hapus-transaksi-{{ $loop->iteration }}" :title="'Hapus Transaksi ' . $item->nomor_transaksi"
        :show="false" focusable>
        <form method="post" action="{{ route('transaksi.destroy', ['transaksi' => $item->slug]) }}" class="p-6">
            @csrf
            @method('delete')
    ...
    <x-modal name="popup-notifikasi" class="flex items-center" :title="'Info!'" :maxWidth="'sm'" :show="$popup">
